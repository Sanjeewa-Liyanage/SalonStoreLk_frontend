...
const DEFAULT_FORM: CreatePlanPayload = {
  planName: '',
  description: '',
  state: PlanState.COMING_SOON,
  price: 0,
  features: [''],
  duration: 30,
  priority: 1,
...
export default function PlanCreateDialog({ open, onClose, onSubmit }: PlanCreateDialogProps) {
  const [form, setForm] = useState<CreatePlanPayload>(DEFAULT_FORM);
  ...
  return (
    <Dialog open={open} onClose={saving ? undefined : resetAndClose} maxWidth="md" fullWidth>
      <DialogTitle sx={{ m: 0, p: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Typography variant="h6" component="div">Create Plan</Typography>
        <IconButton onClick={resetAndClose} size="small" disabled={saving}>
          <CloseIcon />
        </IconButton>
      </DialogTitle>
      ...
      <DialogActions>
        <Button onClick={resetAndClose} disabled={saving}>Cancel</Button>
